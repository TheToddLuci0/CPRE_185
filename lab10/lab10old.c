...
//Beginning of sentance manipulation
		...
		if (ub == 1 && lastTrue != &ub){
			...
			if (RTRUE == 1){
				putchar (toupper (sentance[tempLen-sentMods[modNum] + 1 ]));
				RTRUE=0;
			}
				modNum++;		
		}
			...
			strcat(sentance, wordlist[wordNum]);
			sentMods[modNum] = strlen(wordlist[wordNum]);
			lastTrue = &lb;
			timer2 = 1;
			modNum++;
		...
		*/
		...
		if(WORDADDED == 1){
			...
			tempLen = strlen(sentance);
			mvprintw(26, 0, "%s", sentance);
			mvprintw(26, tempLen, "%s", "                           ");
